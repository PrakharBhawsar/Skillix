...
function NotesModulate(props) {
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        let res = await axios.get("http://localhost:3001/auth/getNote/" + id);
        if (res.data) {
  ...
  const handlerequest = async (id, Statusmsg) => {
    ...
    try {
      let res = await axios.post(
        "http://localhost:3001/UpdateStatus/note",
        data
      );
      if (res) {
